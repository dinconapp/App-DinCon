...
def list_investments(user_id: str = Query(...), db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
...
def get_investment(investment_id: str, db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(repositories(db)["savings"].get(investment_id).user_id, authenticated_user_id)
    return use_cases(db).get_investment(investment_id)


@router.post("/investments", response_model=SavingsInvestmentOut)
def create_investment(payload: SavingsInvestmentCreate, db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
...
def update_investment(investment_id: str, payload: SavingsInvestmentUpdate, db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(repositories(db)["savings"].get(investment_id).user_id, authenticated_user_id)
    return use_cases(db).update_investment(investment_id, payload)


@router.delete("/investments/{investment_id}")
def delete_investment(investment_id: str, db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(repositories(db)["savings"].get(investment_id).user_id, authenticated_user_id)
    return use_cases(db).delete_investment(investment_id)


@router.get("/dashboard", response_model=SavingsDashboardOut)
def dashboard(user_id: str = Query(...), months: int = Query(default=12, ge=1, le=120), db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(user_id, authenticated_user_id)
    return use_cases(db).dashboard(user_id, months)


@router.get("/projection", response_model=list[SavingsProjectionPointOut])
def projection(user_id: str = Query(...), months: int = Query(default=12, ge=1, le=120), db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(user_id, authenticated_user_id)
...
def investment_projection(investment_id: str, months: int = Query(default=12, ge=1, le=120), db: Session = Depends(get_db), authenticated_user_id: str = Depends(require_auth_user)):
    assert_user_access(repositories(db)["savings"].get(investment_id).user_id, authenticated_user_id)
