...
export default function LoginPage() {
  ...
  useEffect(() => {
    const rememberedEmail = window.localStorage.getItem("dincon.remembered.email");
    if (rememberedEmail) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (rememberMe) {
        window.localStorage.setItem("dincon.remembered.email", email.trim().toLowerCase());
      } else {
        window.localStorage.removeItem("dincon.remembered.email");
      }
  ...
  return (
    <AuthShell
      compact
      title="Entrar"
      subtitle="Acesse sua área financeira com segurança."
      footer={<><Link href="/cadastro">Criar conta</Link><Link href="/esqueci-senha">Esqueci minha senha</Link></>}
    >
      {!showVerify ? (
