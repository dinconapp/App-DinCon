  ...
  return (
    <div className="cf-grid">
      ...
      <Card title="Lançamentos">
        ...
        {!tx.loading && !tx.items.length && <EmptyState message="Nenhum lançamento encontrado com os filtros atuais." />}
      ...
      {editing !== undefined && <TransactionFormModal userId={userId} monthKey={monthKey} categories={categories} initial={editing} onClose={() => setEditing(undefined)} onSave={async (payload, id) => { await tx.save(payload, id); onDone("Lançamento salvo com sucesso."); }} />}
    </div>
  );
}
